...
const PIPELINE_REPO_URL =
  "https://github.com/CodyCBakerPhD/aind-ephys-pipeline";
...
function dandiBaseUrl(dandisetId) {
  return SANDBOX_DANDISETS.has(dandisetId)
...
async function fetchDandiAssetId(dandisetId, subject, session) {
  ...
  try {
    ...
    const sourcedataAssetId = await queryPath(
      `sourcedata/sub-${subject}/${nwbName}`,
    );
    if (sourcedataAssetId)
      return { assetId: sourcedataAssetId, inSourcedata: true };
    return null;
  } catch {
    return null;
  }
...
function parseRunPath(runPath) {
  ...
  const pipelineName = pipeMatch
    ? pipeMatch[1]
    : parts[4].replace(/^pipeline-/, "");
...
function renderVizSection(vizByRecording) {
  ...
  const sections = recordings
    .map((rec) => {
      ...
      const recLabel = rec
        .replace(
          /block(\d+)_acquisition-(\w+)_recording(\d+)/,
          "Block $1 · $2 · Recording $3",
        )
...
function renderDandisets(runs) {
  ...
  return dandisetIds
    .map((id) => renderDandisetGroup(id, byDandiset.get(id)))
    .join("");
}
...
function renderSubjectGroup(dandisetId, subject, runs) {
  ...
  const location = rep.inSourcedata
    ? `sourcedata/sub-${subject}`
    : `sub-${subject}`;
...
function renderSessionGroup(dandisetId, session, runs) {
  ...
  const byPipeline = groupBy(
    runs,
    (r) => `${r.pipelineName}\x00${r.pipelineVersion}`,
  );
...
function initModal() {
  ...
  document.addEventListener("keydown", (evt) => {
    if (evt.key === "Escape" && !document.getElementById("log-modal").hidden)
      closeLogModal();
  });
...
function initInlineHtmlFrames() {
  const frames = Array.from(
    document.querySelectorAll("iframe[data-srcdoc-path]"),
  );
...
window.addEventListener('message',function(e){if(e.source===window.parent&&e.data&&e.data.type==='requestHeight')send();});
  ...
  window.addEventListener("message", (evt) => {
    if (
      ...
      (evt.origin !== "null" && evt.origin !== window.location.origin)
    )
      return;
  ...
  document.querySelectorAll("details").forEach((details) => {
    ...
    details.addEventListener("toggle", () => {
      ...
      requestAnimationFrame(() => {
        details
          .querySelectorAll("iframe[data-srcdoc-path]")
          .forEach((iframe) => {
  ...
  Promise.all(
    frames.map(async (iframe) => {
      ...
      const adjustedBodyClose =
        bodyClose !== -1 ? bodyClose + styleToInject.length : -1;
      patched =
        adjustedBodyClose !== -1
          ? patched.slice(0, adjustedBodyClose) +
            heightScript +
            patched.slice(adjustedBodyClose)
